...
async function getAudio() {
  ...
  bufferLength = analyzer.frequencyBinCount; //create specific arrays to contain the data from the analyzer
...
function drawTimeData(timeData) {
  ...
  analyzer.getByteTimeDomainData(timeData); //now that we have the data, let turn it into something visual
  //clear the canvas on every frame

  ctx.clearRect(0, 0, WIDTH, HEIGHT); //setup some canvas drawing
  ...
  ctx.stroke(); // call itself as soon as possbile
...
function drawFrequency(frequencyData) {
  ...
  frequencyData.forEach(amount => {
    ...
    const barHeight = HEIGHT * percent / 2; //convert to colour to HSL todo
